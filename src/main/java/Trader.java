...
public class Trader<T> {
     ...
     */
    public void addToWishlist(T object_o){
        // adds object to the wishlist
        this.wishlist.add(object_o);
    }
     ...
     */
    public int getSellingPrice(T object_o){
        // return the object's price if it is tradable
        if (object_o instanceof Tradable){
            return ((Tradable) object_o).getPrice();
        }
        else{
            return Tradable.MISSING_PRICE;
        }
